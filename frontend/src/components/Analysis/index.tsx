...
type AnalysisModule = 'hub' | 'earned-premium' | 'claims' | 'data-sorting' | 'revised-composite' | 'alm-cashflow' | 'pmp-monitoring' | 'reserving';
...
export function Analysis({ portfolioId }: AnalysisProps) {
  ...
  if (activeModule === 'data-sorting') {
    return <DataSorting onBack={handleBackToHub} />;
  }
  ...
  return (
    <div className="container" style={{ animation: 'fadeIn 0.4s ease-out', paddingTop: '1.75rem' }}>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(280px, 1fr))', gap: '1.25rem' }}>
        ...
        <div 
          className="glass-panel analysis-module-card" 
          onClick={() => setActiveModule('data-sorting')}
          style={{ cursor: 'pointer', padding: '1.5rem', transition: 'all 0.2s', border: '1px solid transparent' }}
        >
          <div style={{ marginBottom: '1rem' }}>
            <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" strokeWidth={1.5} stroke="currentColor" width="32" height="32" style={{ color: 'var(--primary)' }}>
          ...
          <h3 style={{ fontSize: '0.9rem', fontFamily: 'monospace', textTransform: 'uppercase', letterSpacing: '0.1em', marginBottom: '0.5rem', color: 'var(--primary)' }}>Data Sorting</h3>
          <p style={{ color: 'var(--text-muted)', lineHeight: '1.5', fontSize: '0.85rem' }}>
            Upload and process Fincon & Production Report data for the month.
          </p>
        </div>
